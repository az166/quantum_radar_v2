...
class TradingPerformanceLogger:
    ...
    def log_entry_signal(self, symbol, entry_price, score, action, z_score, btc_risk_status, tp_level, cl_level):
        log_entry = {
    ...
    def close_logged_signal(self, signal_id, exit_price, current_time=None):
        ...
        try:
            with open(self.log_filepath, 'r+') as f:
                ...
                updated = False
                for entry in data:
                    if entry["signal_id"] == signal_id and entry["status"] == "OPEN":
                        entry["status"] = "CLOSED"
            ...
            print(f"Error closing logged signal: {e}")
...
def verify_breakout_status(live_price, last_close, open_price, high_price, low_price, local_swing_high, z_score):
    ...
    candle_range = max(0.00001, high_price - low_price)
...
async def process_single_coin_pipeline(client, symbol, m_data, user_portfolio, semaphore, state_manager, device_id="default_guest_device"):
    async with semaphore:
        ...
        try:
            ...
            vol_spike_ratio = live_volume / vol_ma20 if vol_ma20 > 0 else 0.0
            ...
            # 2. Analisis Lanjutan Market Structure & Breakout Status
            market_struct, last_sh, last_sl = analyze_market_structure(klines_1h, window=5)
            ...
            # 3. Model Engine 4 Level Risiko BTC
            btc_risk = calculate_btc_risk_level(state_manager.btc_status, btc_returns_snapshot)

            # 4. Asosiasi Keputusan Melalui Confidence Score Engine
